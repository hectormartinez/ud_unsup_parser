...
do
    cd src
    python udup.py --steps $s --input ../data/orgtok/$postype/$domain-ud-test.conllu --lang $domain
    cd ..
    perl eval07.pl -g data/orgtok/domains/$domain-ud-test.conllu -s src/$domain"_testout.conllu" > "$domain.domains.eval"
done
